...
export function translatePoints(points: AnnotationPoint[], dTime: number, dPrice: number): AnnotationPoint[] {
  return points.map((point) => ({ time: point.time + dTime, price: point.price + dPrice }));
}

export function movePoint(points: AnnotationPoint[], index: number, point: AnnotationPoint): AnnotationPoint[] {
