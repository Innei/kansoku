...
function validateAnnotation(item: unknown): asserts item is Annotation {
  ...
  if (!KINDS.includes(a.kind)) throw new ClientError(`annotation ${id}: kind must be one of ${KINDS.join(", ")}`);
  ...
  if (typeof a.createdAt !== "number") throw new ClientError(`annotation ${id}: createdAt must be a number`);
