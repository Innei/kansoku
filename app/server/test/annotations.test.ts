...
describe("PUT /:symbol validation", () => {
  ...
  it("accepts an annotation with source, label, and style set", async () => {
    const annotations = [
      makeAnnotation({ source: "ai", label: "breakout level", style: { color: "#A855F7", width: 2, dash: true } }),
    ];
    const res = await tsukiRequest("/api/annotations/NVDA.US", {
      method: "PUT",
      headers: { "content-type": "application/json" },
  ...
  it("rejects a non-boolean style.dash, naming the id and field", async () => {
    const res = await tsukiRequest("/api/annotations/NVDA.US", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ annotations: [makeAnnotation({ style: { dash: "yes" as unknown as boolean } })] }),
    });
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toContain("ann-1");
  ...
  it("rejects a source outside user|ai, naming the id and field", async () => {
    const res = await tsukiRequest("/api/annotations/NVDA.US", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ annotations: [makeAnnotation({ source: "bot" as Annotation["source"] })] }),
    });
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toContain("ann-1");
  ...
  it("accepts a polyline with 20 points", async () => {
    const points = Array.from({ length: 20 }, (_, i) => ({ time: 1700000000 + i * 100, price: 100 + i }));
    const res = await tsukiRequest("/api/annotations/NVDA.US", {
      method: "PUT",
      headers: { "content-type": "application/json" },
  ...
  it("rejects a polyline with 1 point", async () => {
    const res = await tsukiRequest("/api/annotations/NVDA.US", {
      ...
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        annotations: [makeAnnotation({ kind: "polyline", points: [{ time: 1700000000, price: 100 }] })],
      }),
    });
    expect(res.status).toBe(400);
  });
  ...
  it("rejects a polyline with 21 points", async () => {
    const points = Array.from({ length: 21 }, (_, i) => ({ time: 1700000000 + i * 100, price: 100 + i }));
    const res = await tsukiRequest("/api/annotations/NVDA.US", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ annotations: [makeAnnotation({ kind: "polyline", points })] }),
  ...
  it("rejects a non-boolean style.arrow, naming the id and field", async () => {
    const res = await tsukiRequest("/api/annotations/NVDA.US", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ annotations: [makeAnnotation({ style: { arrow: "yes" as unknown as boolean } })] }),
    });
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toContain("ann-1");
