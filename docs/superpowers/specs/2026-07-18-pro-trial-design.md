...
## UI 引导（2026-07-18 追加，已实施）

引导漏斗从纯被动改为三层曝光，全部只在「装了 pro 且未授权」时出现，激活后自动消失：

1. **新手引导第四步**：桌面端 onboarding 在「连接数据 → 配置 AI → 连接 X」之后追加「Kansoku AI」步骤，直接复用付费墙组件（`Paywall` 从 `LicenseModal.tsx` 导出），可激活可跳过（「跳过，先免费使用」）；已授权或无 pro slot 时整步不出现（`Onboarding.tsx` + `StepPro.tsx`）。
2. **首页常驻入口**：QuickBar 图标区新增 ✨ 按钮（accent 色，`quickbar-trial`），点击打开付费墙。
3. **设置页复用付费墙**：「还没有授权码？免费试用 7 天」从直链月付结账改为打开付费墙 modal——年付方案因此在设置页入口也可见。

付费墙 modal（`LicenseModal.tsx`）保持唯一销售面：三卖点 + 月付试用 CTA + 年付链接 + 激活表单。
