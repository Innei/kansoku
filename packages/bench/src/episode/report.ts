...
function renderTradeLedger(row: ReportRow, chartId: string): string {
  const trades = closedTrades(row.answer);
  if (trades.length === 0) {
...
function renderCaseDetail(row: ReportRow, index: number): string {
  ...
  return `<article class="trade-case" id="case-${index}" data-model="${escapeHtml(answer.model)}" data-mode="${escapeHtml(answer.mode)}" data-outcome="${escapeHtml(outcome)}">
    <header class="case-head"><div><h3>${escapeHtml(answer.symbol)}${row.provenance ? ` <span class="provenance-alias">→ ${escapeHtml(row.provenance.sourceSymbol)}</span>` : ''}</h3><span>${escapeHtml(answer.questionId)} · ${escapeHtml(answer.model)} · ${escapeHtml(MODE_LABELS[answer.mode] ?? answer.mode)}</span>${row.provenance ? `<small class="provenance-line">${escapeHtml(formatProvenanceLine(row.provenance))}</small>` : ''}<small class="trace-ref" title="Trace file relative to the run directory"><span>Trace</span> <code>${escapeHtml(answer.traceRef)}</code></small></div><div class="case-result"><span class="status ${valueClass(result?.netR)}">${escapeHtml(TERMINATION_LABELS[outcome] ?? outcome)}</span><strong class="${valueClass(result?.netR)}">${escapeHtml(fmtSigned(result?.netR, 3))} R</strong></div></header>
    <div class="case-layout"><section class="chart-panel"><div class="chart-toolbar"><div><strong>K 线与成交量</strong><span>点击工具节点可回看该 B 编号当时可见的数据</span></div><div class="timeframe-tabs" role="tablist" aria-label="K 线周期">${available.map((tf, i) => `<button type="button"${i === 0 ? ' class="active"' : ''} data-timeframe-tab data-chart="trade-chart-${index}" data-timeframe="${tf}">${CHART_TIMEFRAME_LABEL[tf]}</button>`).join('')}</div></div><div class="tv-chart" id="trade-chart-${index}" data-chart-id="trade-chart-${index}"><span class="chart-loading">加载图表…</span></div><div class="chart-legend"><span><i class="entry"></i>计划入场</span><span><i class="target"></i>止盈</span><span><i class="stop"></i>止损</span><span><i class="decision"></i>决策位置</span><span><i class="ema20"></i>EMA20</span><span class="chart-range" data-chart-range="trade-chart-${index}"></span></div>${renderProcessChain(row, index, available, defaultTimeframe)}</section>
      ...
      ${renderTradeLedger(row, `trade-chart-${index}`)}<details class="actions"><summary>回放动作与理由 <span>${actions.length}</span></summary>${actions.length === 0 ? `<p>没有动作记录</p>` : `<ol>${actions.map(renderActionRecord).join('')}</ol>`}</details></aside></div>
  </article>`;
}
...
const SCRIPT = String.raw`
  (() => {
    ...
    function renderChart(id, timeframe, requestedBarIndex) {
      ...
      if (data.length >= 20) {
        const emaAlpha = 2 / (20 + 1);
        let seed = 0;
        for (let i = 0; i < 20; i += 1) seed += data[i].close;
        seed = seed / 20;
        const emaPoints = [];
        let prev = seed;
        for (let i = 19; i < data.length; i += 1) {
          const value = i === 19 ? seed : data[i].close * emaAlpha + prev * (1 - emaAlpha);
          prev = value;
          emaPoints.push({ time: data[i].time, value });
        }
        const emaSeries = chart.addSeries(library.LineSeries, {
          color: '#f59e0b',
          lineWidth: 1,
    ...
    document.querySelectorAll('[data-trade-jump]').forEach((element) => {
      element.addEventListener('click', () => {
        const chartEl = document.getElementById(element.dataset.chart);
        if (chartEl) chartEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
        const cp = payloads.get(element.dataset.chart);
        renderChart(element.dataset.chart, element.dataset.timeframe || cp?.defaultTimeframe || 'h1', element.dataset.barIndex);
      });
    });
