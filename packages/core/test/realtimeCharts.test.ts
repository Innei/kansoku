...
describe('subscribeChart candlestick-push wiring', () => {
  ...
  beforeEach(() => {
    ...
    longbridgeStream.subscribeCandlesticks.mockImplementation(
      (_symbol: string, period: string, cb: CandleCb) => {
  ...
  it("writes the poller's freshly fetched bars into candle state so a later push has no hole", async () => {
    ...
    build.buildChart.mockResolvedValue({
      input: {
        timeframes: {
          m5: [
            { time: new Date(5_000).toISOString(), open: 5, high: 5, low: 5, close: 5, volume: 5 },
          ],
          m15: [],
          h1: [],
        },
        ...
        input: {
          ...
          timeframes: {
            m5: [
              ...
              {
                time: new Date(3_000).toISOString(),
                open: 3,
                high: 3,
                low: 3,
                close: 3,
...
describe('subscribePreview', () => {
  ...
  beforeEach(() => {
    ...
    build.refreshBody
      .mockReset()
      .mockImplementation((_type: string, input: { symbol: string }) => ({
        type: 'intraday',
        symbol: input.symbol,
      }));
    build.rebuild
      .mockReset()
      .mockImplementation((_type: string, input: { timeframes?: { m5?: unknown[] } }) => ({
        type: 'intraday',
        title: '预览',
        symbol: 'PREVIEW.US',
        input: {},
