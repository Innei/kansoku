...
describe('longbridgeProvider (CLI-backed)', () => {
  ...
  it('prefers the WS transport for kline and quotes when available', async () => {
    ...
    const rows = [
      { symbol: 'NVDA.US', last: '110', prev_close: '100', change_percentage: '10.000' },
    ];
    const transport = {
  ...
  it('skips the CLI fallback when the WS auth is rejected for quota exhaustion', async () => {
    const transport = {
      queryQuotes: vi
        .fn()
        .mockRejectedValue(new Error('Longbridge response failed: command=2 status=5')),
      queryCandlesticks: vi.fn().mockRejectedValue(new Error('socket down')),
      queryCapitalFlow: vi.fn(),
      queryCapitalDistribution: vi.fn(),
      queryStaticNames: vi.fn(),
  ...
  it('arms the quota cooldown when the CLI itself hits the connection limit', async () => {
    ...
    const run = vi
      .fn()
      .mockRejectedValue(new Error('connections limitation is hit, limit = 10, online = 10'));
    const provider = createLongbridgeProvider(run as LongbridgeRunner, () => transport);
  ...
  it('maps capital, positions, portfolio, and deduplicated watchlist commands', async () => {
    ...
    await expect(provider.getPortfolio!()).resolves.toMatchObject({
      holdings: [{ symbol: 'NVDA.US' }],
    });
  ...
  it('maps CLI failures to the existing ClientError envelope', async () => {
    const provider = createLongbridgeProvider(
      vi.fn().mockRejectedValue(new Error('not logged in')),
    );
    await expect(provider.getQuotes(['NVDA.US'])).rejects.toMatchObject({
      status: 502,
